...
from foodscrape.models.trie import Trie
...
class Processor:
    def __init__(self):
        ...
        self.model_filename = Config.MODEL_DIR / "ingredient_model.pkl"
        self.model_data_dir = Config.BASE_DIR / "foodscrape/models/data"
    ...
    def _process_data(self) -> pd.Series:
        food_df: pd.DataFrame = pd.read_pickle(self.model_data_dir / "ingr_map.pkl")
        food_df = food_df.rename(columns={"replaced": "name"})
        food_df = food_df.drop(
            columns=["raw_ingr", "raw_words", "processed", "len_proc", "count", "id"]
        )
    ...
    def _filter_by_count(self, df: pd.DataFrame, min_count: int) -> pd.DataFrame:
        mask = df["count"] >= min_count
        return df.loc[mask]

    def _filter_by_length(self, df: pd.DataFrame, min_length: int) -> pd.DataFrame:
